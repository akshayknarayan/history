...
func TestAddOne(t *testing.T) {
	dummyItem := struct{}{}
	lg := MakeUniqueHistory(time.Duration(10) * time.Millisecond)
...
func TestTimeLimit(t *testing.T) {
	lg := MakeUniqueHistory(time.Duration(5) * time.Millisecond)
...
func TestCountLimit(t *testing.T) {
	lg := MakeUniqueHistory(time.Duration(1) * time.Millisecond)
